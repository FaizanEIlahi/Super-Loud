...
def get_msa_oews():
    if settings.ENVIRONMENT == 'prod':
        ...
        schema = 'dev'
    cur = connection.cursor()
    cur.execute("select distinct msa_code, msa_title from {}.s_dropdown_state_msa_oews".format(schema))
    a = cur.fetchall()
    retval = {}
    for i in a:
        retval[i[0]] = i[1]
...
def get_msa_from_state_occ_from_db(state):
    if settings.ENVIRONMENT == 'prod':
        schema = 'prod'
    else:
        schema = 'dev'
    ...
    cur = connection.cursor()
    cur.execute("select msa_code, msa_title from {}.s_dropdown_state_msa_oews where state_id={}".format(schema, state))
    a = cur.fetchall()
    retval = []
    for i in a:
        b = {}
...
def get_skills_from_msa_from_db(msa):
    ...
    query = "select distinct tech_skills from {}.msa_occ_skills_20221222 where msa_code = '".format(schema) + msa + "' order by tech_skills"
    print(query)
